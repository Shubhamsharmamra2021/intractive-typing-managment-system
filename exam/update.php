...
if(@$_GET['demail'] && $_SESSION['key']=='shubham') {
$demail=@$_GET['demail'];
...
//remove quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'rmquiz' && $_SESSION['key']=='shubham') {
$eid=@$_GET['eid'];
...
//add quiz
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='shubham') {
$name = $_POST['name'];
...
//add question
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'addqns' && $_SESSION['key']=='shubham') {
$n=@$_GET['n'];
$eid=@$_GET['eid'];
$ch=@$_GET['ch'];
...
{
$sn++;
header("location:loginstudent.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
else if( $_SESSION['key']!='shubham')
{
...
header("location:loginstudent.php?q=result&eid=$eid");
}
else
{
header("location:loginstudent.php?q=result&eid=$eid");
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:loginstudent.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
