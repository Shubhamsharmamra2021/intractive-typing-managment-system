...
</br></br>Fast and Finger :<span  style="font-weight: bold; color:green;"> <span id="ff">00</span> Wpm.</span> 
</div>
<button style="margin-left: 250px; margin-bottom: 200px; margin-top: 100px; 
  position: relative;
   height: 60px;
  width: 400px;
  display: block;
  ...
  background: linear-gradient(to left, #111111 0%, #DC143C 100%);
  color: #fff;
  font-size: 18px;
  font-weight: 500;
  ...
  box-shadow: 0 4px 15px rgba(0,0,0,0.2);"  id="btnDone" >Done</button>
 												<a href="/login.php#loginform"><button style="margin-left: 250px; margin-bottom: 200px; margin-top: 100px; 
  position: relative;
   height: 60px;
  width: 400px;
  display: none;
...
var cps=(((totalkey-totalbk)/(totalTime*60)).toFixed(2));
 var ff=((((totalkey-totalbk)/5)/totalTime).toFixed(0));
	...
	var return2 = "That's " + (wordsCorrect/totalTime).toFixed(0) + " words per minute at " + percentAccuracy + " % accuracy";
